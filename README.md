...
`/restaurants` 收錄新竹市區、竹北、竹東與關埔・科學園區共 38 間餐廳，可依下列條件交叉篩選：
...
每個料理類型 × 每段價位帶至少保留 2 間選擇，共覆蓋 21 個交叉組合。每張餐廳卡片包含營業時間、概估價位、地址、交通建議、Google Maps 與資料來源；部分店家另附第三方平台評價快照。新增或調整餐廳後，請執行 `npm run test:regression`，確認 21 個組合、區域數量、價位帶、篩選邏輯與靜態頁面輸出仍一致。
